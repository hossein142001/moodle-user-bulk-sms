...
class user_bulk_new_form extends moodleform {
    function definition() {
        ...
        $this->add_action_buttons(false, 'ارسال پیامک');
...
class user_sms_form extends moodleform {


    function definition() {
    
        $mform = & $this->_form;
        $mform->addElement('header', 'general', get_string("pluginname" , "local_sms"));

        // Sms text
        $mform->addElement('text', 'text', get_string("messagebody" ), array('size' => '60'));
        $mform->addRule('text', '', 'required', null, 'client');

        $sms = new sms();
        $mform->addElement('static', 'description',get_string("remind_credit" , "local_sms" , [$sms->get_customer_balance()]) );
